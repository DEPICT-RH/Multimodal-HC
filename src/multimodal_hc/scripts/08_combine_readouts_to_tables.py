...
if not os.path.exists(df_path) or not os.path.exists(df_path_input_function):
    ...
    df_input_functions = pd.read_csv(df_path_input_function)
    df_organs = pd.read_csv(df_path)
...
# %%
if not os.path.exists(df_path := READOUTS_ROOT/"means.csv"):
    data = []
...
if not os.path.exists(df_path := READOUTS_ROOT/"patlak_ki.csv"):
